...
> from the prior project's research log, now at legacy/phase0/RESEARCH.md.)

## Project

Clean-room recreation of the thought-vector idea from `legacy/phase0/RESEARCH.md`
...
## 2026-07-04 — CONSOLIDATED_RESEARCH.md absorbed into this log; repo restructured

Repo root moved up from `thoughtvec/` to the project root; earlier project
generations preserved under `legacy/` (phase0 = original thought-vectors
prototype, phase1 = BitThought rebuild, git history in
`legacy/phase1-git-history.bundle`). `CONSOLIDATED_RESEARCH.md` is deleted;
its per-experiment content lives in `legacy/phase0/RESEARCH.md` (the full
Phase 1 log, verified byte-identical to the former top-level RESEARCH.md) and
in this file (Phase 2). Its cross-phase synthesis is recorded below so
nothing is lost.

### Phase 1 key insights (2025-05, from legacy/phase0/RESEARCH.md)

1. GRU > self-attention for thought generation — sequential ordering is
   essential for prefix-based compression (self-attention failed to converge).
2. GRU → cross-attn ordering is optimal (V1 beat V2 by ~5%): the GRU builds a
   scaffold that cross-attention fills.
3. d_model=256 was the efficiency sweet spot for datasets <1M texts (later
   shown to be an undertraining artifact — see Phase 2 rationale below).
4. 4+4 layers beat 6+6 at that scale.
5. 128 thoughts >>> 32 thoughts for compression; 16 vectors from 128 thoughts
   ≈ full reconstruction of a 62-token article (~4:1).
6. d_model=256 bottlenecks 100+ token texts — 50–62% word-overlap ceiling.
7. VAE regularization costs no compression quality — smooth latent space free.
8. NAR training pushes the ceiling higher — +13% word overlap at 122 tokens;
   blended k-sampling gained +19% at 6:1.
9. Deep-narrow GRU > shallow-wide GRU (256 unique steps beat 32×8 expansion).
10. The decode-time bottleneck is the real compression mechanism.
11. Predictor needs length-relative thresholds (loss_at_k / loss_at_full).
12. Data quality trumps model architecture at this scale.
13. Capping inference k to training max_k prevents attention dilution.
14. Phase 1 thinker runs: thinker_conv_s1 (10K UltraChat, S0 loss 6.94) →
    thinker_big_s1 (104K all sources, 3.41); clean data was the main lever.
15. Residuals then: refusal patterns inherited from safety-trained data;
    factual confusion attributable to d=256 capacity.

### Phase 1 checkpoint index (files remain in legacy/, weights not tracked)

| Model | d | Layers | Thoughts | Loss | File |
|---|---|---|---|---|---|
| C4 baseline | 256 | 4+4 | — | 1.99 | c4_256d_4x4_10ep_C4.pt |
| Compression (128-thought) | 256 | 4+4 | 128→16 min | 0.115 | compressed_128t.pt |
| VAE+NAR compression | 256 | 4+4 | 256→var | 0.08 | vae_compressed_nar.pt |
| Blend compression | 256 | 4+4 | 256→var | 0.09 | vae_compressed_blend.pt |
| Thinker chat (104K) | 256+6L | 4+4+think | 256→auto | 3.41 (S0) | thinker_big_s1.pt |
| Thinker (three-phase) | 256+6L | 4+4+think | 256→auto | ~5.09 | thinker_three_phase.pt |

### Cross-phase big picture (as of the consolidation)

Phase 1 proved the idea: GRU-scaffolded thought vectors, prefix compression,
and a latent-space thinker — but hit a d=256 ceiling and a hard-to-tune
three-phase schedule. Phase 2's rationale was that those limits were
artifacts: (i) "d=256 beats 512/768" came from 10-epoch post-norm runs with
no warmup on 100K sentence fragments; (ii) refinements (VAE, NAR, blended-k)
were applied sequentially as fine-tunes instead of jointly from step 0;
(iii) the corpus was sentence fragments (median ~14 words). Phase 2 broke
three ceilings: d384 5+5 single-phase training, per-sample-k + monotone
